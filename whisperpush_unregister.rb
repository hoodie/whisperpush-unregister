...
def get_code(phone_number)
  uri = URI.parse("https://whisperpush.cyanogenmod.org/v1/accounts/sms/code/#{phone_number}")
...
def validate_code(phone_number, verification_code, push_password, aes_key, hmac_key, registration_id)
  uri = URI.parse("https://whisperpush.cyanogenmod.org/v1/accounts/code/#{verification_code}")
  pp uri
  http = Net::HTTP.new(uri.host, uri.port)
  http.use_ssl = true
  http.verify_mode = SSL_VERIFY_MODE
...
def unregister_number(phone_number, push_password, push_type, num_rounds=20)
  if ((push_type == :gcm) or (push_type == :apn))
    uri = URI.parse("https://whisperpush.cyanogenmod.org/v1/accounts/#{push_type}")
  else
...
OptionParser.new do |opts|
  opts.banner = "Usage: ./whisperpush_unregister.rb [options]\n\n" +
                "EXAMPLES:\n" +
...
if options.execution_mode == :getconfirm
  ...
  if result[0..1] != "20"
    ...
    puts "You should now be unregistered from CyanogenMod's WhisperPush service."
  end
end
